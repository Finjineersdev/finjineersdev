# 👋 Hi, I'm Haseeb  
Senior Mobile App Engineer • Flutter • React Native • Kotlin • Swift

<p align="left">
  <img src="https://komarev.com/ghpvc/?username=Finjineersdev&label=Profile%20Views&color=0e75b6&style=flat" />
</p>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&pause=1000&center=true&vCenter=true&width=700&lines=Senior+Mobile+App+Engineer;Flutter+%7C+React+Native;Kotlin+%7C+Swift;Clean+Architecture+%26+Scalable+Apps;Performance+Focused+Mobile+Engineering" />
</p>

---

## 🧭 Profile Summary

**Senior Mobile App Engineer** with **5+ years of experience** delivering **production-grade mobile applications** used by real users at scale.

I work across **Flutter**, **React Native**, **Kotlin**, and **Swift**, focusing on **clean architecture**, **performance optimization**, and **maintainable codebases**. I’m comfortable joining **existing large codebases**, leading features end-to-end, and collaborating with cross-functional teams.

✔ Ideal for **recruiters** seeking a strong individual contributor  
✔ Ideal for **clients** looking for reliable, long-term mobile expertise  

---

## 💼 What I Bring (Recruiter-Friendly)

- Strong experience with **large, long-running projects**
- Clean Architecture, MVVM, feature-based structuring
- Confident in **code reviews & refactoring**
- Excellent debugging & problem-solving skills
- Clear communication & documentation habits
- Comfortable with **remote & distributed teams**

---

## 🚀 What Clients Get

- Production-ready mobile apps (not prototypes)
- Pixel-perfect UI from Figma
- Smooth scrolling & optimized performance
- Secure API & Firebase integrations
- Reliable delivery & maintainable code

---

## 🛠️ Core Tech Stack

### 📱 Mobile Platforms
<p>
  <img src="https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter&logoColor=white" />
  <img src="https://img.shields.io/badge/React_Native-20232A?style=for-the-badge&logo=react&logoColor=61DAFB" />
  <img src="https://img.shields.io/badge/Kotlin-7F52FF?style=for-the-badge&logo=kotlin&logoColor=white" />
  <img src="https://img.shields.io/badge/Swift-F54A2A?style=for-the-badge&logo=swift&logoColor=white" />
</p>

### 🧱 Architecture & State
<p>
  <img src="https://img.shields.io/badge/Clean_Architecture-4CAF50?style=for-the-badge" />
  <img src="https://img.shields.io/badge/MVVM-000000?style=for-the-badge" />
  <img src="https://img.shields.io/badge/BLoC-2E3440?style=for-the-badge&logo=flutter" />
  <img src="https://img.shields.io/badge/Riverpod-007ACC?style=for-the-badge&logo=flutter" />
</p>

### 🔗 Backend & Services
<p>
  <img src="https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black" />
  <img src="https://img.shields.io/badge/REST_APIs-005571?style=for-the-badge" />
</p>

---

## 🔥 Key Strengths

✔ Flutter performance optimization  
✔ Large list handling & pagination  
✔ State consistency across complex flows  
✔ Error handling & retry strategies  
✔ Secure authentication flows  
✔ Feature ownership from design → release  

---

## 🏆 GitHub Trophies

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=Finjineersdev&theme=onestar&no-frame=true&row=1&column=7" />
</p>

---

## 📊 GitHub Activity

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Finjineersdev&show_icons=true&theme=transparent" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Finjineersdev&theme=transparent" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Finjineersdev&layout=compact&theme=transparent" />
</p>

---

## 📫 Contact & Availability

- 💼 LinkedIn: https://www.linkedin.com/
- 📧 Email: mhaseeb.it@gmail.com
- 🌍 Open to: Remote roles • Contract • Long-term projects

---

> “Clean architecture makes scaling teams and apps effortless.”

⭐ Follow for Flutter architecture, performance tuning & real-world mobile engineering.
